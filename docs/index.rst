...
Censored Planet data is available to the public through the Censored Planet website: `data.censoredplanet.org/raw <https://data.censoredplanet.org/raw>`_. The Censored Planet raw data website contains archived compressed data files corresponding to one scan using each measurement technique, following by the `ISO Alpha-2 <https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2>`_country code. 
...
Each scan contains a test of all 2,000 websites tested by Censored Planet and is split into country-specific files . The archive files can be downloaded, extracted and analyzed. The data formats change based on the version of the measurement technique. The data formats and tips for analyzing the data for each of the published data files and versions are available below. It is critical that the raw data produced by Censored Planet is treated as a network observation and analyzed further using our `analysis pipeline <https://github.com/censoredplanet/censoredplanet-analysis>`_ before use. For more information about using the data, please refer to the `Censored Planet Github <https://github.com/censoredplanet/censoredplanet>`_, or email Censored Planet at `censoredplanet@umich.edu <mailto:censoredplanet@umich.edu>`_. 
